...
def demarrer_jeu(difficulte):
    print(f"Difficulté sélectionnée : {difficulte}")
    hangman()
    pass
...
def hangman():
    ...
    if difficulte == 1:
        mots = [m for m in mots if len(m) <= 5]
    elif difficulte == 2:
        mots = [m for m in mots if 5 < len(m) <= 8]
    elif difficulte == 3:
        mots = [m for m in mots if len(m) > 8]
...
menu.add.button('HANGMAN', lambda: demarrer_jeu(difficulte))
